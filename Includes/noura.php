<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Noura -kleren</title>
    <!-- ============== Bootstrap CSS File Link============= -->

    <link rel="stylesheet" href="../bootstrap/css/bootstrap.css">

    <!-- ================= Custom CSS File=================== -->

    <link rel="stylesheet" href="../CSSFiles/noura.css">
</head>
<body>
<?php   
  include("header.php");
  ?>
    <section class="noura" id="noura">
    <div class="container p-4">
        <div class="links mb-3">
            <a href="" class="text-dark text-decoration-none fs-4 text-uppercase">Home /</a>
            <a href="" class="text-dark text-decoration-none fs-4 text-uppercase">Winter Collections /</a>
            <a href="" class="text-dark text-decoration-none fs-4 text-uppercase">khaddar /</a>
            <a href="" class="text-dark text-decoration-none fs-4 text-uppercase">Noura : Emboirdered Khaddar
            </a>
        </div>
        <div class="row section-heading mb-3">
            <h1 class="main-heading text-center  fw-light text-dark text-uppercase mt-5 mb-5"><span>Noura: Emboirdered Khaddar</span></h1>
        </div>


        <div class="row mb-4">
   <div class="col-md-4 col-sm-6 mb-4">
    <div class="image">
        <a href=""><img src="../images/no1.webp" alt="pic"class="img-fluid w-100"></a>
    </div>
    <div class="text">
        <h3 class="text-uppercase fs-4 mt-2 mb-2">noura no-01 :Emboirdered Khaddar </h3>
        <p class="text-uppercase fs-3 text-dark mb-4"><span class="text-decoration-line-through">rs 4,999.00  </span><sub>RS3,999.00</sub></p>
        <a href="" class="cart text-white text-decoration-none text-capitalize fs-3 fw-light">add to cart</a>
    </div>
   </div>
   <div class="col-md-4 col-sm-6 mb-4">
    <div class="image">
        <a href=""><img src="../images/no2.webp" alt="pic"class="img-fluid w-100"></a>
    </div>
    <div class="text">
        <h3 class="text-uppercase fs-4 mt-2 mb-2">noura no-02 :Emboirdered Khaddar </h3>
        <p class="text-uppercase fs-3 text-dark mb-4"><span class="text-decoration-line-through">rs 4,999.00  </span><sub>RS3,999.00</sub></p>
        <a href="" class="cart text-white text-decoration-none text-capitalize fs-3 fw-light">add to cart</a>
    </div>
   </div>
 
   <div class="col-md-4 col-sm-6 mb-4">
    <div class="image">
        <a href=""><img src="../images/no3.webp" alt="pic"class="img-fluid w-100"></a>
    </div>
    <div class="text">
        <h3 class="text-uppercase fs-4 mt-2 mb-2">noura no-03 :Emboirdered Khaddar</h3>
        <p class="text-uppercase fs-3 text-dark mb-4"><span class="text-decoration-line-through">rs 4,999.00  </span><sub>RS3,999.00</sub></p>
        <a href="" class="cart text-white text-decoration-none text-capitalize fs-3 fw-light">add to cart</a>
    </div>
   </div>
        </div>
        <div class="row mb-4">
   <div class="col-md-4 col-sm-6 mb-4">
    <div class="image">
        <a href=""><img src="../images/no4.webp" alt="pic"class="img-fluid w-100"></a>
    </div>
    <div class="text">
        <h3 class="text-uppercase fs-4 mt-2 mb-2">noura no-0d :Emboirdered Khaddar</h3>
        <p class="text-uppercase fs-3 text-dark mb-4"><span class="text-decoration-line-through">rs54,999.00  </span><sub>RS3,999.00</sub></p>
        <a href="" class="cart text-white text-decoration-none text-capitalize fs-3 fw-light">add to cart</a>
    </div>
   </div>
   <div class="col-md-4 col-sm-6 mb-4">
    <div class="image">
        <a href=""><img src="../images/no5.webp" alt="pic"class="img-fluid w-100"></a>
    </div>
    <div class="text">
        <h3 class="text-uppercase fs-4 mt-2 mb-2">noura no-02 :Emboirdered Khaddar</h3>
        <p class="text-uppercase fs-3 text-dark mb-4"><span class="text-decoration-line-through">rs 4,999.00  </span><sub>RS3,999.00</sub></p>
        <a href="" class="cart text-white text-decoration-none text-capitalize fs-3 fw-light">read more...</a>
    </div>
   </div>
   <div class="col-md-4 col-sm-6 mb-4">
    <div class="image">
        <a href=""><img src="../images/no6.webp" alt="pic"class="img-fluid w-100"></a>
    </div>
    <div class="text">
        <h3 class="text-uppercase fs-4 mt-2 mb-2">noura no-03 :Emboirdered Khaddar</h3>
        <p class="text-uppercase fs-3 text-dark mb-4"><span class="text-decoration-line-through">rs 4,999.00  </span><sub>RS3,999.00</sub></p>
        <a href="" class="cart text-white text-decoration-none text-capitalize fs-3 fw-light">read more...</a>
    </div>
   </div>
        </div>

        <div class="row mb-4">
   <div class="col-md-4 col-sm-6 mb-4">
    <div class="image">
        <a href=""><img src="../images/no7.webp" alt="pic"class="img-fluid w-100"></a>
    </div>
    <div class="text">
        <h3 class="text-uppercase fs-4 mt-2 mb-2">noura no-07 :Emboirdered Khaddar</h3>
        <p class="text-uppercase fs-3 text-dark mb-4"><span class="text-decoration-line-through">rs 4,999.00  </span><sub>RS3,999.00</sub></p>
        <a href="" class="cart text-white text-decoration-none text-capitalize fs-3 fw-light">read more...</a>
    </div>
   </div>
   <div class="col-md-4 col-sm-6 mb-4">
    <div class="image">
        <a href=""><img src="../images/no8.webp" alt="pic"class="img-fluid w-100"></a>
    </div>
    <div class="text">
        <h3 class="text-uppercase fs-4 mt-2 mb-2">noura no-08 :Emboirdered Khaddar</h3>
        <p class="text-uppercase fs-3 text-dark mb-4"><span class="text-decoration-line-through">rs 4,999.00  </span><sub>RS3,999.00</sub></p>
        <a href="" class="cart text-white text-decoration-none text-capitalize fs-3 fw-light">read more...</a>
    </div>
   </div>
   <div class="col-md-4 col-sm-6 mb-4">
    <div class="image">
        <a href=""><img src="../images/no9.webp" alt="pic"class="img-fluid w-100"></a>
    </div>
    <div class="text">
        <h3 class="text-uppercase fs-4 mt-2 mb-2">noura no-09 :Emboirdered Khaddar</h3>
        <p class="text-uppercase fs-3 text-dark mb-4"><span class="text-decoration-line-through">rs 4,999.00  </span><sub>RS3,999.00</sub></p>
        <a href="" class="cart text-white text-decoration-none text-capitalize fs-3 fw-light">read more...</a>
    </div>
   </div>
   <div class="col-md-4 col-sm-6 mb-4">
    <div class="image">
        <a href=""><img src="../images/no10.webp" alt="pic"class="img-fluid w-100"></a>
    </div>
    <div class="text">
        <h3 class="text-uppercase fs-4 mt-2 mb-2">noura no-10 :Emboirdered Khaddar</h3>
        <p class="text-uppercase fs-3 text-dark mb-4"><span class="text-decoration-line-through">rs 4,999.00  </span><sub>RS3,999.00</sub></p>
        <a href="" class="cart text-white text-decoration-none text-capitalize fs-3 fw-light">read more...</a>
    </div>
   </div>
   <div class="col-md-4 col-sm-6 mb-4">
    <div class="image">
        <a href=""><img src="../images/no11.webp" alt="pic"class="img-fluid w-100"></a>
    </div>
    <div class="text">
        <h3 class="text-uppercase fs-4 mt-2 mb-2">noura no-10 :Emboirdered Khaddar</h3>
        <p class="text-uppercase fs-3 text-dark mb-4"><span class="text-decoration-line-through">rs 4,999.00  </span><sub>RS3,999.00</sub></p>
        <a href="" class="cart text-white text-decoration-none text-capitalize fs-3 fw-light">read more...</a>
    </div>
   </div>
   <div class="col-md-4 col-sm-6 mb-4">
    <div class="image">
        <a href=""><img src="../images/no12.webp" alt="pic"class="img-fluid w-100"></a>
    </div>
    <div class="text">
        <h3 class="text-uppercase fs-4 mt-2 mb-2">noura no-10 :Emboirdered Khaddar</h3>
        <p class="text-uppercase fs-3 text-dark mb-4"><span class="text-decoration-line-through">rs 4,999.00  </span><sub>RS3,999.00</sub></p>
        <a href="" class="cart text-white text-decoration-none text-capitalize fs-3 fw-light">read more...</a>
    </div>
   </div>
   </div>
        </div>
        <div class="row mt-3 mb-3">
            <h1 class="text-uppercase text-center mb-2" style="color:rgb(5, 5, 53); font-size:3rem;">looking for more?</h1>
            <a href="" class="text-white text-uppercase fs-2 fw-light text-center  p-2 m-auto text-decoration-none" style="background:rgb(5, 5, 53); width:300px;">view all collections</a>
        </div>

    </div>
    </section>
    <?php   
  include("footer.php");
  ?>

       <!-- =============== Ionicons JS Links==================== -->
<script type="module" src="https://unpkg.com/ionicons@5.5.2/dist/ionicons/ionicons.esm.js"></script>
<script nomodule src="https://unpkg.com/ionicons@5.5.2/dist/ionicons/ionicons.js"></script>

   <!-- ===================== Bootstrap JS File Link=================== -->

    <script src="../bootstrap/js/bootstrap.js"></script>
</body>
</html>